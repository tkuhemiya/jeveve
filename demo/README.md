...
`GET /health` only means the web process is up, not that an extractor is warm. The first extract for a model boots that pool (often >150s). Modal then 303s to a result URL; the client uses `httpx` with `follow_redirects=True` and retries `503` + `Retry-After`. Click **Ignite small + base + multi** once before running the catalog.
